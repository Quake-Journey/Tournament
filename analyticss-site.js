...
if (!SITE_CHAT_ID.length) {
  console.warn('WARNING: SITE_CHAT_ID is empty or invalid. No chats will be available.');
}

const app = express();

// Просто чтобы можно было положить сюда стили или картинки, если захочешь
...
(async () => {
  try {
    ...
    // Если в URI задана база, db() возьмёт её, иначе можно указать явно
    db = client.db();
...
app.get('/', async (req, res) => {
  try {
    ...
    const queryChatId = req.query.chatId ? Number(req.query.chatId) : null;
